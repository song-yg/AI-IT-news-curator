...
** 시트 형식 (첫 행은 헤더) **
  ...
  - active: TRUE/FALSE - 사람이 행을 삭제하지 않고 켜고 끌 수 있게 함 (실수로 지워서 이력이 사라지는 것보다, 꺼두고 note에 이유를 남기는 쪽이 안전)
  - note: 자유 메모(왜 껐는지 등) - 코드는 안 읽음, 사람이 보는 용도

** 실패 시 fallback **
KEYWORD_SHEET_CSV_URL 환경변수가 없거나, 네트워크 실패, 시트 형식이 깨졌거나, 특정 lang에 활성 키워드가 하나도 없는 경우 등
- 어떤 이유로든 정상적으로 못 읽으면 호출부가 넘겨준 fallback(각 collector에 원래 있던 하드코딩 리스트)으로 안전하게 대체한다.
즉 이 기능을 아예 설정 안 해도, 설정했다가 시트가 일시적으로 안 열려도 파이프라인 자체는 죽지 않는다.
"""
...
# 프로세스 내 캐시.
# get_keywords가 "ko"/"en" 각각을 위해 독립적으로 호출됨(naver_collector가 ko용, gdelt_collector가 en용).
...
def _fetch_csv_rows(csv_url: str) -> list[dict] | None:
    ...
    try:
        # 네트워크/구글 서버 쪽 일시적 지연으로 인해 30초로 넉넉하게 잡음.
...
def _is_valid_keyword(keyword: str) -> bool:
    """
    키워드가 실제 검색어로 쓸 만한지(문자/숫자를 하나라도 포함하는지) 확인한다. 즉, 특수문자만 존재하는 경우 제거.
    """
...
def get_keywords(lang: str, fallback: list[str]) -> list[str]:
    ...
    # 중복 키워드 제거
    # - 시트에 같은 키워드가 실수로 두 번 이상 등록되면 naver_collector/gdelt_collector가 그 키워드로 API를 중복 호출하게 돼서, 여기서 한 번만 걸러내면 모든 호출부가 자동으로 혜택을 본다.
    # 공백 차이나 대소문자 차이도 실질적으로 같은 검색어이므로 정규화해서 비교하고, 실제 검색에 쓰는 표기는 시트에 먼저 등장한 쪽을 그대로 유지한다.
    seen_normalized = set()
    deduped_keywords = []
    duplicates = []
    for kw in keywords:

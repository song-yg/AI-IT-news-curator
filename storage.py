...
def save_scored(directory: str, domestic_summarized: list[dict],
    ...
    """
    스코어링+요약 결과, 카테고리 전체 집계, 실패 소스, GDELT 시계열 참고 지표를 scored.json 하나로 묶어 저장한다.

    category_distribution을 여기 같이 저장해두는 이유는 category_aggregator.py 모듈 docstring에 명시돼 있음.
     - 다음 주 실행에서 "지난주 대비 증감"을 계산하려면 지난주 집계 결과가 파일로 남아있어야 하기 때문(이슈 그룹핑 자체는 매주 새로 도니, 주 단위 비교는 이 파일을 통해서만 가능).

    카테고리별 Top N(국내/해외 각각 {카테고리: [항목, ...]})도 domestic_by_category/international_by_category로 같이 저장한다.
     - scorer.score_by_category()가 이미 각 항목에 "category" 필드를 남겨두므로(main.py의 _regroup_by_category 참고) 여기서는 그대로 저장만 하면 됨.
       "category_distribution"(단순 집계 개수)과 이름이 헷갈리지 않게 구분해서 키를 붙임.
        - category_distribution은 개수만 세는 3-보조 지표이고, 이건 카테고리별 실제 Top N 이슈 목록이라 성격이 다름.

    save_raw와 동일한 이유로 파일 쓰기 실패를 안전하게 흡수한다(로그 + None 반환).
    """
    payload = {
...
def _format_category_comparison_section(category_comparison: dict[str, dict[str, dict]] | None) -> list[str]:
    """
    category_aggregator.compare_with_last_week()의 결과를 summary.md용 마크다운 줄 리스트로 만든다.
    콘솔의 category_aggregator. print_aggregate_with_comparison()과 같은 정보를 담는다 - 이슈 목록보다 위, 문서 맨 앞부분에 배치해서 "이번 주 큰 흐름"을 먼저 보여주는 구성.

    category_comparison이 None이면(지난주 데이터 없음) 빈 리스트 반환 - 호출부가 이 경우 섹션 자체를 아예 안 넣도록.
    """
...
def save_summary_md(directory: str, week_label: str, domestic_summarized: list[dict],
    ...
    """
    사람이 바로 읽을 배포용 요약본. llm_summarizer.print_summaries와 같은 내용을 마크다운 파일로 남긴다(요약 유무와 무관하게 원문 링크는 항상 같이 남김).
    문서 작업 시 언더바(_) 등 앞의 이스케이프는 넣지 않는다(마크다운 렌더링 시 불필요한 백슬래시가 그대로 노출되는 문제 방지 - 프로젝트 방침).

    save_raw/save_scored와 동일한 이유로 파일 쓰기 실패를 안전하게 흡수한다(로그 + None 반환).

    국내/해외 각 섹션 밑에 "카테고리별 Top N" 하위 섹션을 추가한다(##### 대신 #### 레벨 - 국내/해외(##)보다 한 단계, 개별 이슈 제목(###)과도 겹치지 않게 구분).
    카테고리가 하나도 없으면(이번 주 그 축에 "기타" 아닌 카테고리 이슈가 전혀 없었던 경우) 하위 섹션 자체를 생략한다.

    category_comparison(카테고리별 지난주 대비 증감, category_aggregator.compare_with_last_week() 결과)이 있으면 문서 맨 앞(생성 시각 바로 다음)에 "카테고리별 지난주 대비 증감" 섹션을 추가한다.
...
def save_week(articles: list[dict], domestic_summarized: list[dict],
    ...
    """
    main.py에서 부르는 단일 진입점. raw.json/scored.json/summary.md를 한 디렉토리에 다 저장하고 그 디렉토리 경로를 반환한다.

    디렉토리 생성 자체가 실패하면 저장을 아예 포기하고 None을 반환한다(로그는 week_dir이 이미 남김).
    디렉토리는 만들어졌는데 파일 하나가 실패하는 경우엔 나머지 파일 저장은 계속 시도하고, 끝나고 나서 뭐가 저장되고 뭐가 안 됐는지 요약 로그를 남긴다(부분 성공도 사람이 바로 알 수 있게).

    domestic_by_category/international_by_category(카테고리별 Top N, main.py의 step4_category_llm_summary 결과)도 같이 받아서 scored.json/summary.md에 반영한다.
    """
